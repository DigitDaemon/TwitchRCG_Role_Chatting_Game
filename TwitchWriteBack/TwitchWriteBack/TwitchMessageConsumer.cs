...
namespace TwitchWriteBack
{
    class TwitchMessageConsumer
    {
         ...
         */
        public TwitchMessageConsumer(ref List<KeyValuePair<string,ConcurrentQueue<string>>> MessageQueues)
        {
        ...
        public void TMCThread()
        {
            ...
            while (Active)
            {
                ...
                try
                {
                    ...
                    {
                        MessageQueues.Find(x => x.Key.Equals(consumerresult.Message.Key)).Value.Enqueue(consumerresult.Message.Value);
                    }
                    else
                    {
                        Thread.Sleep(100);
